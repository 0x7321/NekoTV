...
const PlatformCard: React.FC<PlatformCardProps> = ({
  ...
  const handleViewMore = () => {
    ...
    sessionStorage.setItem(
      `discover_${cateId}`,
      JSON.stringify({ name: categoryName, pic: backgroundImage })
    );
    // 跳转到详情页
  ...
  return (
    <div
      className='platform-card min-w-[340px] h-[300px] rounded-[18px] p-[18px] flex flex-col gap-4 relative overflow-hidden'
      style={{
    ...
    >
      ...
      <div className='relative z-10 flex gap-2'>
        {topItems.slice(0, 3).map((item, index) => (
          <div
            key={index}
            className={`flex-1 h-[140px] rounded-xl overflow-hidden bg-black/40 rank-${
              index + 1
            }`}
          >
            <img
