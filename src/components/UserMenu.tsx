...
export const UserMenu: React.FC = () => {
  ...
  // 加载播放记录（优化版 - 使用 requestIdleCallback）
  useEffect(() => {
    if (
      typeof window !== 'undefined' &&
      authInfo?.username &&
      ...
      // 🔥 性能优化：使用 requestIdleCallback 延迟加载，避免阻塞主线程
      ...
      if ('requestIdleCallback' in window) {
        ...
        // 降级：使用 setTimeout
        const timeoutId = setTimeout(loadPlayRecords, 100);
        idleCallbackId = timeoutId as unknown as number;
      }
      ...
      return () => {
        ...
        if ('cancelIdleCallback' in window && idleCallbackId) {
          ...
          clearTimeout(idleCallbackId as unknown as NodeJS.Timeout);
        }
